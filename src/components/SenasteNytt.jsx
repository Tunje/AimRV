...
const SenasteNytt = () => {
      ...
      try {
        ...
        const posts = postsData.map((post) => {
          ...
          if (data.image) {
            ...
            const img = new Image();
            img.onload = () =>
              console.log(`Image for post ${post.id} loaded successfully`);
            img.onerror = () =>
              console.error(`Image for post ${post.id} failed to load`);
            img.src = data.image;
          } else {
  ...
  const handleDeletePost = async (postId) => {
    try {
      ...
      const postToDelete = newsPosts.find(
  ...
  return (
    <div className="flex flex_col">
      {/* Hero Image Section - only background image */}
      <section
        className="page_01 senaste-nytt-hero dynamic-background background-editable"
        id="senaste-nytt-top-background"
      ></section>

      {/* Title Section - separate from hero */}
      <section className="senaste-nytt-title-section">
        <div className="title-container">
          <EditableText
            textKey="senaste-nytt-main-title"
            defaultText="SENASTE NYTT"
            tag="h1"
            className="senaste-nytt-main-title"
          />
        </div>
      </section>
      ...
      <div className="set-03 senaste-nytt-subscribe-section">
        <div className="senaste-nytt-container flex flex_col flex_j_C flex_a_C font_white">
          ...
          <form
            ...
            className="senaste-nytt-form flex flex_col flex_a_C"
          >
              ...
              .map((post) => (
                <div key={post.id} className="senaste-nytt-news-card">
                  ...
                  <div className="senaste-nytt-news-content">
                    <h3 className="senaste-nytt-news-card-title">
                      {post.title}
                    </h3>
                    <p className="senaste-nytt-news-date">{post.date}</p>
                    {!post.published && isAdmin && (
                      <p className="senaste-nytt-unpublished-notice">
                        Opublicerad
                      </p>
                    )}
                    <div
                    ...
                    <Link
                      to={`/news/${post.id}`}
                      className="senaste-nytt-read-more"
                    >
                      {readMoreText}
                    </Link>
                    {isAdmin && (
                      <div className="senaste-nytt-admin-buttons">
                        ...
                        <button
                          className={`senaste-nytt-publish-post-button ${
                            post.published ? "published" : "unpublished"
                          }`}
                          onClick={() =>
                            handleTogglePublish(String(post.id), post.published)
                          }
                        >
      ...
      <div
        id="senaste-nytt-background"
        className="background-editable senaste-nytt-background-container locked-background page_01"
      ></div>
    </div>
  );
};
