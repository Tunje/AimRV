...
const SponsorEditor = ({ location }) => {
  ...
  const [newSponsor, setNewSponsor] = useState({ imageUrl: '', link: '', location: '' });
  ...
  const currentLocationName = locationPages.includes(currentPage) ? currentPage : '';
  ...
  useEffect(() => {
    ...
    addEditButton('#sponsors-section', 'sponsor-edit-button', 'Edit Sponsors', false);
    
    // Add edit button for local partners if we're on a location page
    if (currentLocationName) {
      addEditButton('.sponsors-title', 'local-sponsor-edit-button', 'Edit Local Partners', true);
    }
  ...
  const loadSponsors = async () => {
    try {
      ...
      if (isLocalPartners && currentLocationName) {
        ...
        sponsorsData = sponsorsData.filter(sponsor => sponsor.location === currentLocationName);
      } else {
  ...
  const displaySponsors = (sponsorsData) => {
    ...
    if (isLocalPartners && currentLocationName) {
      ...
      container = document.querySelector('.sponsors-section .sponsors-container');
    } else {
    ...
    sponsorsData.forEach(sponsor => {
      ...
      if (sponsor.link) {
        ...
        linkElement.href = sponsor.link.startsWith('http') ? sponsor.link : `https://${sponsor.link}`;
  ...
  const handleAddSponsor = async () => {
    ...
    try {
      ...
      // Update local state
      const updatedSponsors = [...sponsors, { id: newSponsorRef.id, ...sponsorData }];
  ...
  return (
    <>
      {showModal && (
        <div className="background-editor-overlay" onClick={(e) => {
          ...
          <div className="background-editor-modal" ref={modalRef}>
            <div className="background-editor-modal__header">
              <h2 className="background-editor-modal__title">
                {isLocalPartners ? `Edit ${currentLocationName.charAt(0).toUpperCase() + currentLocationName.slice(1)} Partners` : 'Edit Global Sponsors'}
              </h2>
              <button 
            ...
            <div className="background-editor-modal__body">
              <h3 className="background-editor-modal__subtitle">Current Sponsors</h3>
              <div className="background-editor-modal__ad-list">
                {sponsors.length > 0 ? (
                  sponsors.map((sponsor, index) => (
                    <div key={index} className="background-editor-modal__ad-item">
                      <img src={sponsor.imageUrl} alt={`Sponsor ${index + 1}`} className="background-editor-modal__ad-image" />
                      <div className="background-editor-modal__ad-details">
                        <p className="background-editor-modal__ad-link">{sponsor.link ? (
                  ...
                  <p className="background-editor-modal__empty-message">No sponsors added yet.</p>
                )}
              </div>
              
              <h3 className="background-editor-modal__subtitle">Add New Sponsor</h3>
